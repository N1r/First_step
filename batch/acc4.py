    ...
    try:
        ...
        data = {
            ...
            "messages": [
                ...
                核心目标（Objective）：
                可以标题党，最大化点击率与用户共鸣，同时确保内容政治正确、事实准确、立场坚定——突出中国发展成就，揭露美方战略焦虑，激发民族自豪感与战略自信。符合40岁以上男性喜好的风格

                内容要求（Content Requirements）：
                立场导向：凡涉及中美对比，必须体现“中国稳步发展、美国焦虑失措”的叙事框架。可使用如“破防”“急了”“酸了”“认怂”“被打脸”等具象化情绪词描述美方反应；对中国成就则使用“领先”“突破”“震撼”“自主可控”等正向强化词。
                
                信息准确性：人名（如Lloyd Austin → 劳埃德·奥斯汀）、机构（如Department of Defense → 美国国防部）、装备型号（如F-35、歼-20）、时间地点等关键要素必须严格准确，不得虚构或模糊处理。
                文化适配：将美方政治隐喻或制度性表述（如“great power competition”）转化为中国读者熟悉的语境（如“遏制中国”“围堵战略”），必要时补充背景（如“在珠海航展亮相后”“面对中国高超音速导弹进展”）以增强叙事张力。
                标题结构：采用“冲突+反转”或“美方反应+中方实力”结构，制造认知反差。例如：“美军承认无力拦截 ‖ 中国高超音速导弹令五角大楼彻夜难眠”。

                标题字数控制在20–35字之间，节奏紧凑，关键词前置
                禁止添加解释、注释、引号或其他任何额外字符

                频道名须与原始内容来源一致（如Fox News → Fox News）
                优化指标（Success Metrics）：
                用户点击意愿强（标题具悬念、冲突或情绪钩子）
                政治立场无偏差（坚定维护国家利益与形象）
                专业可信度高（术语准确、逻辑严密、无夸大失实）
...
def create_yaml_config(videos, covers, titles, dtimes, yaml_file):
    ...
    try:
        ...
        print(f"YAML 内容已成功保存到 {YAML_OUTPUT_FILE}")
...
def split_and_create_yaml_dub(videos, covers, titles, dtimes):
    """80% 免费 + 20% 付费 分开生成"""
    total = len(videos)
    indices = list(range(total))
    random.shuffle(indices)
    ...
    free_titles = [titles[i] for i in free_indices]
    #free_dtimes = [dtimes[i] for i in free_indices]
    create_yaml_config(free_videos, free_covers, free_titles, dtimes, 'dub_free_content.yaml')
    ...
    paid_titles = [titles[i] for i in paid_indices]
    #paid_dtimes = [dtimes[i] for i in paid_indices]
    create_yaml_config_charge(paid_videos, paid_covers, paid_titles, dtimes, 'dub_paid_content.yaml')
...
def find_output_with_dub_files(directory):
    """查找指定目录下所有子文件夹中名为 output_with_sub.mp4 的文件"""
    return [os.path.join(root, file) 
            for root, _, files in os.walk(directory) 
            for file in files 
           # if file == 'output_sub.mp4']
            if file == 'output_dub_final.mp4']

def main():
    # 查找封面和视频文件
    covers = find_files_with_suffix(OUTPUT_DIR, COVER_SUFFIX)
    videos = find_output_with_sub_files(OUTPUT_DIR)
    dub_videos = find_output_with_dub_files(OUTPUT_DIR)
    ...
    # 创建YAML配置文件
    dub_titles = ['[中配]'+ i for i in titles]
    split_and_create_yaml(videos, new_covers, titles, dtimes)
    split_and_create_yaml_dub(dub_videos, new_covers, dub_titles, dtimes)
